...
const { checkFriends, startFriendCheckLoop, stopFriendCheckLoop, refreshFriendCheckLoop, getFriendsList, getFriendLandsDetail, doFriendOperation, getFriendScheduleStatus } = require('../services/friend');
...
async function runFarmTick(auto) {
    ...
    try {
        ...
        if (auto.fertilizer_gift && auto.farm_fertilizer_use_gift_pack) await openFertilizerGiftPacksSilently();
...
async function runFriendTick(auto) {
    ...
    try {
        if (auto.friend_steal || auto.friend_water || auto.friend_weed || auto.friend_bug || auto.friend_bad) await checkFriends();
    } catch (e) {
...
function applyRuntimeConfig(snapshot, syncNow = false) {
    ...
    if (loginReady) {
        ...
        if (hasAutomationPayload) {
            ...
            const nextFertilizerGift = !!(nextAuto && nextAuto.fertilizer_gift);
            if (!prevFertilizerGift && nextFertilizerGift) {
...
async function startBot(config) {
    ...
    const onLoginSuccess = async () => {
        ...
        const auto = getAutomation();
        if (auto.fertilizer_gift && auto.farm_fertilizer_use_gift_pack) {
    ...
    // 启动定时状态同步（6 秒间隔降低主进程 CPU，实际发送仍由 hash/8s 节流控制）
    workerScheduler.setIntervalTask('status_sync', 6000, syncStatus, { preventOverlap: true });
}
    ...
    try {
        switch (method) {
            case 'getLands':
                ...
                break;
            case 'getFriends':
                result = await getFriendsList(args[0], args[1]);
                break;
            case 'getFriendLands':
            ...
            case 'getFriendSchedule':
                result = getFriendScheduleStatus();
                break;
            case 'getSeeds':
            ...
            case 'getBag':
                ...
                break;
            case 'useItem': {
                const { useItem: _useItem, batchUseItems: _batchUse } = require('../services/warehouse');
...
function syncStatus() {
    ...
    fullStats.friendSchedule = getFriendScheduleStatus();
    const hash = JSON.stringify(fullStats);
    const now = Date.now();
    if (hash !== lastStatusHash || now - lastStatusSentAt > 10000) {
        lastStatusHash = hash;
