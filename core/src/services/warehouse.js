...
async function getBag() {
    ...
    try {
        ...
        throw new Error(`背包数据解析失败: ${(e && e.message) ? e.message : '未知'}`);
    }
}
...
/**
 * 将前端请求的出售项（可能来自合并后的背包，uid 为 0）解析为真实背包堆栈项。
 * 游戏服务端出售需携带正确的 uid，否则会返回 1000020 请求参数错误。
 */
function resolveSellItemsFromBag(requestedItems, rawBagItems) {
    const stacks = (rawBagItems || []).map(it => ({
        id: toNum(it.id),
    ...
    for (const req of (requestedItems || [])) {
        const wantId = toNum(req.id);
        let wantCount = Math.max(1, toNum(req.count));
        if (wantId <= 0) continue;

        const matching = stacks.filter(s => s.id === wantId);
        for (const stack of matching) {
            if (wantCount <= 0) break;
            const take = Math.min(wantCount, stack.count);
            toSell.push({ id: stack.id, count: take, uid: stack.uid });
            wantCount -= take;
            stack.count -= take;
        }
    }
...
async function sellItems(items) {
    // 若传入项均无有效 uid（来自前端合并展示），则从背包解析真实堆栈
    const hasAnyUid = (items || []).some(it => toNum(it && it.uid) > 0);
    let resolved = items;
    if (!hasAnyUid && items && items.length > 0) {
        try {
            const bagReply = await getBag();
            const rawItems = getBagItems(bagReply);
            resolved = resolveSellItemsFromBag(items, rawItems);
            if (resolved.length === 0) throw new Error('无法从背包解析出售项，请刷新背包后重试');
        } catch (e) {
            throw new Error((e && e.message) ? e.message : '解析出售项失败');
        }
    }

    const payload = resolved.map(toSellItem);
...
async function getBagDetail() {
    ...
    try {
        ...
        return { totalKinds: 0, items: [], _error: (e && e.message) ? e.message : '获取背包失败' };
    }
    try {
        const rawItems = getBagItems(bagReply);
        const merged = new Map();
    ...
    return { totalKinds: items.length, items };
        ...
        return { totalKinds: 0, items: [], _error: (e && e.message) ? e.message : '处理背包数据失败' };
    }
}
...
async function sellAllFruits() {
    ...
    try {
        ...
        while (Date.now() - startWait < 2000) {
            const state = getUserState();
            const currentGold = (state && state.gold != null) ? state.gold : goldAfter;
