...
/**
 * 从成长任务中解析所需种子 ID（用于优先购买/种植）
 * 规则：未完成且未领取的任务，从 params[0] 或 params 中首个数字解析为 seed_id
 * @returns {Promise<{ seedId: number, desc: string } | null>}
 */
async function getGrowthTaskRequiredSeedId() {
    try {
        ...
        for (const t of growthList) {
            if (t.is_claimed) continue;
            const totalProgress = toNum(t && t.total_progress);
            const progress = toNum(t && t.progress);
            if (totalProgress <= 0 || progress >= totalProgress) continue;
            ...
            const desc = (t && t.desc) || `成长任务#${toNum(t && t.id)}`;
            return { seedId, desc };
        }
        return null;
    } catch {
