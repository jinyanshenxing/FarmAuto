...
const { isAutomationOn, getPreferredSeed, getAutomation, getPlantingStrategy, getPlantOrderRandom, getFertilizerOrderRandom, getPlantDelaySeconds } = require('../models/store');
...
const { getGrowthTaskRequiredSeedId } = require('./task');
...
/**
 * 施肥 - 必须逐块进行，服务器不支持批量
 * 间隔由配置 automation.fertilizer_interval_normal_ms 控制，默认 50ms
 */
async function fertilize(landIds, fertilizerId = NORMAL_FERTILIZER_ID) {
    const intervalMs = Math.max(0, Number(getAutomation().fertilizer_interval_normal_ms) || 50);
    let successCount = 0;
    for (const landId of landIds) {
        ...
        if (landIds.length > 1 && intervalMs > 0) await sleep(intervalMs);
    }
    return successCount;
}
 ...
 * 间隔由配置 automation.fertilizer_interval_organic_ms 控制，默认 100ms
 */
async function fertilizeOrganicLoop(landIds) {
    const ids = (Array.isArray(landIds) ? landIds : []).filter(Boolean);
    if (ids.length === 0) return 0;

    const intervalMs = Math.max(0, Number(getAutomation().fertilizer_interval_organic_ms) || 100);
    let successCount = 0;
    let idx = 0;
...
async function runFertilizerByConfig(plantedLands = []) {
    if (!isAutomationOn('fertilizer_gift')) return { normal: 0, organic: 0 };

    let planted = (Array.isArray(plantedLands) ? plantedLands : []).filter(Boolean);
    const fertilizerOrderRandom = getFertilizerOrderRandom();
    if (fertilizerOrderRandom && planted.length > 1) {
        planted = shuffleArray(planted);
    }
    ...
    // 自动普通肥料：种植后第一时间施普通肥
    if (isAutomationOn('farm_fertilizer_normal') && planted.length > 0) {
    ...
    // 自动施一次有机肥：种植后执行施一次有机肥
    if (isAutomationOn('farm_fertilizer_organic_once')) {
        let organicTargets = planted.length > 0 ? planted.slice() : [];
        if (organicTargets.length === 0) {
            ...
            try {
                const latest = await getAllLands();
                const allOrganic = getOrganicFertilizerTargetsFromLands(latest && latest.lands);
                const plantedSet = new Set(planted);
                organicTargets = allOrganic.filter(id => plantedSet.has(id));
            } catch (_) {
                organicTargets = [];
            }
        }
        if (fertilizerOrderRandom && organicTargets.length > 1) {
            organicTargets = shuffleArray(organicTargets);
        }
        if (organicTargets.length > 0) {
            fertilizedOrganic = await fertilize(organicTargets, ORGANIC_FERTILIZER_ID);
            if (fertilizedOrganic > 0) {
                log('施肥', `已为 ${fertilizedOrganic} 块地施一次有机肥`, {
                    module: 'farm',
                    event: 'fertilize',
                    result: 'ok',
                    type: 'organic_once',
...
/**
 * 种植 - 间隔由设置 plantDelaySeconds 控制（0=默认50ms）
 */
async function plantSeeds(seedId, landIds) {
    const delaySec = (typeof getPlantDelaySeconds === 'function' ? getPlantDelaySeconds() : 0) || 0;
    const delayMs = delaySec > 0 ? Math.min(60000, Math.round(delaySec * 1000)) : 50;
    let successCount = 0;
    for (const landId of landIds) {
        try {
...
/**
 * 获取背包中某种子数量（种子在背包中的 item.id 即 seed_id）
 */
async function getSeedCountInBag(seedId) {
    try {
        const bagReply = await getBag();
        const items = getBagItems(bagReply) || [];
        let count = 0;
        const sid = Number(seedId) || 0;
        for (const it of items) {
            if (toNum(it && it.id) === sid) count += toNum(it && it.count) || 0;
        }
        return count;
    } catch {
        return 0;
...
/**
 * 在种子商店中查找指定 seedId 的商品（用于成长任务优先购买）
 * @returns {{ goodsId: number, seedId: number, price: number, requiredLevel: number } | null}
 */
async function findSeedInShop(seedId) {
    ...
    try {
        ...
        for (const goods of shopReply.goods_list) {
            ...
            if (toNum(goods.item_id) !== targetId) continue;
            ...
            if (state.level < requiredLevel) continue;
...
async function autoPlantEmptyLands(deadLandIds, emptyLandIds) {
    ...
    // 2. 优先成长任务：若存在未完成的成长任务且需要某作物，优先购买/种植该作物
    const needCount = landsToPlant.length;
    let bestSeed = null;
    let fromGrowthTask = false;
    try {
        const growthReq = await getGrowthTaskRequiredSeedId();
        if (growthReq && growthReq.seedId) {
            const seedInShop = await findSeedInShop(growthReq.seedId);
            if (seedInShop) {
                bestSeed = seedInShop;
                fromGrowthTask = true;
                log('商店', `成长任务优先: ${growthReq.desc} → 种子 ${getPlantNameBySeedId(growthReq.seedId)} (${growthReq.seedId})`, {
                    module: 'farm', event: 'seed_pick', seedId: growthReq.seedId, scope: 'growth_task'
                });
            }
        }
        if (!bestSeed) {
    ...
    // 3. 购买：成长任务优先时若背包已有该种子则只补足差额
    let buyCount = needCount;
    if (fromGrowthTask) {
        const bagCount = await getSeedCountInBag(bestSeed.seedId);
        buyCount = Math.max(0, needCount - bagCount);
        if (buyCount <= 0) {
            log('商店', `成长任务种子已有 ${bagCount} 个，无需购买`, {
                module: 'warehouse', event: 'seed_buy_skip', result: 'enough_in_bag', seedId: bestSeed.seedId
            });
        }
    }

    const totalCost = bestSeed.price * buyCount;
    if (buyCount > 0 && totalCost > state.gold) {
        ...
        if (canBuy <= 0) {
            if (fromGrowthTask && (await getSeedCountInBag(bestSeed.seedId)) >= needCount) {
                buyCount = 0;
            } else {
                return;
            }
        } else {
            const newBuyCount = Math.min(buyCount, canBuy);
            const plantCount = fromGrowthTask ? Math.min(needCount, (await getSeedCountInBag(bestSeed.seedId)) + newBuyCount) : canBuy;
            buyCount = newBuyCount;
            landsToPlant = landsToPlant.slice(0, plantCount);
            if (!fromGrowthTask) log('商店', `金币有限，只种 ${plantCount} 块地`);
        }
    }
    ...
    // 4. 种植（可选随机地块顺序，间隔由 plantDelaySeconds 控制）
    if (typeof getPlantOrderRandom === 'function' && getPlantOrderRandom()) {
        for (let i = landsToPlant.length - 1; i > 0; i--) {
            const j = Math.floor(Math.random() * (i + 1));
            [landsToPlant[i], landsToPlant[j]] = [landsToPlant[j], landsToPlant[i]];
        }
    }
    let plantedLands = [];
    try {
...
function analyzeLands(lands) {
    const result = {
        ...
        harvestableInfo: [],
        /** 生长中地块的剩余秒数直到成熟 [{ landId, remainSec }] */
        growingRemainSec: [],
    };
    ...
    for (const land of lands) {
        ...
        if (maturePhase) {
            ...
            const remainSec = matureBegin > nowSec ? matureBegin - nowSec : 0;
            result.growingRemainSec.push({ landId: id, remainSec });
        }
    }
...
async function runFarmOperation(opType) {
    ...
    // 通知好友模块：我的土地已拉取，用于检测「好友偷取我」并打日志
    try {
        const { networkEvents } = require('../utils/network');
        if (networkEvents && typeof networkEvents.emit === 'function') networkEvents.emit('myLandsFetched', lands);
    } catch (_) { /* ignore */ }
    ...
    // 执行除草/虫/水（受自动设置 farm_weed / farm_bug / farm_water 控制）
    if (opType === 'all' || opType === 'clear') {
        if (isAutomationOn('farm_weed') && status.needWeed.length > 0) {
        ...
        if (isAutomationOn('farm_bug') && status.needBug.length > 0) {
        ...
        if (isAutomationOn('farm_water') && status.needWater.length > 0) {
    ...
    if (isAutomationOn('fertilizer_gift') && isAutomationOn('farm_fertilizer_anti_steal') && Array.isArray(status.growingRemainSec) && status.growingRemainSec.length > 0) {
        const auto = getAutomation();
        const thresholdSec = Math.max(0, Number(auto.farm_fertilizer_anti_steal_sec) || 300);
        const antiStealLandIds = status.growingRemainSec
            .filter(r => r.remainSec > 0 && r.remainSec <= thresholdSec)
            .map(r => r.landId);
        if (antiStealLandIds.length > 0) {
            try {
                const latest = await getAllLands();
                const organicTargets = getOrganicFertilizerTargetsFromLands(latest && latest.lands);
                const organicSet = new Set(organicTargets);
                const toFert = antiStealLandIds.filter(id => organicSet.has(id));
                if (toFert.length > 0) {
                    const n = await fertilize(toFert, ORGANIC_FERTILIZER_ID);
                    if (n > 0) {
                        actions.push(`防偷肥${n}`);
                        log('施肥', `防偷: 为 ${n} 块地施有机肥加速成熟`, {
                            module: 'farm', event: 'fertilize', result: 'ok', type: 'anti_steal', count: n,
                        });
                        recordOperation('fertilize', n);
                    }
                }
            } catch (e) {
                logWarn('施肥', `防偷施肥失败: ${e.message}`);
            }
        }
    }

    // 施肥细分：连续施有机肥（按策略间隔）
    if (isAutomationOn('fertilizer_gift') && isAutomationOn('farm_fertilizer_organic_loop')) {
        try {
            const latest = await getAllLands();
            let organicTargets = getOrganicFertilizerTargetsFromLands(latest && latest.lands);
            if (organicTargets.length > 0) {
                if (getFertilizerOrderRandom() && organicTargets.length > 1) {
                    organicTargets = shuffleArray(organicTargets);
                }
                const count = await fertilizeOrganicLoop(organicTargets);
                if (count > 0) {
                    actions.push(`有机肥${count}`);
                    log('施肥', `连续有机肥完成 ${count} 次`, {
                        module: 'farm', event: 'fertilize', result: 'ok', type: 'organic_loop', count,
                    });
                    recordOperation('fertilize', count);
                }
            }
        } catch (e) {
            logWarn('施肥', `连续有机肥失败: ${e.message}`);
        }
    }
