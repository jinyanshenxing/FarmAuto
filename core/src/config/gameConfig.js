...
function getCropHarvestValue(plantId) {
    const plant = plantMap.get(Number(plantId) || 0);
    if (!plant || !plant.fruit) return 0;
    const fruitId = Number(plant.fruit.id) || 0;
    const fruitCount = Number(plant.fruit.count) || 0;
    if (!fruitId || !fruitCount) return 0;
    const unitPrice = getFruitPrice(fruitId);
    return fruitCount * unitPrice;
}
