...
function createDataProvider(options) {
    ...
    return {
        ...
        purgeLogsForAccount: (accountId) => (purgeLogsForAccount && purgeLogsForAccount(accountId)),

        // 透传方法
        getLands: (accountRef) => callWorkerApi(resolveAccountRefId(accountRef), 'getLands'),
        getFriends: (accountRef, page, pageSize) => callWorkerApi(resolveAccountRefId(accountRef), 'getFriends', page, pageSize),
        ...
        saveSettings: async (accountRef, payload) => {
            ...
            const snapshot = {
                ...
                friendOpOrder: body.friendOpOrder,
                plantOrderRandom: body.plantOrderRandom,
                fertilizerOrderRandom: body.fertilizerOrderRandom,
