...
function createWorkerManager(options) {
    ...
    function stopWorker(accountId) {
        ...
        // 立即从 map 移除，使 GET /api/accounts 立刻显示已停止，避免“界面已停止但脚本仍在跑”的错觉
        delete workers[accountId];
        proc.send({ type: 'stop' });
        managerScheduler.setTimeoutTask(`force_kill_${accountId}`, 1000, () => {
            try {
                if (proc && (typeof proc.exitCode !== 'number' && proc.exitCode !== null))
                    proc.kill();
            } catch {}
        });
    }
    ...
    function handleWorkerMessage(accountId, msg) {
        ...
        if (msg.type === 'status_sync') {
            ...
            if (worker.logs.length > 400) worker.logs.splice(0, worker.logs.length - 400);
            globalLogs.push(logEntry);
            if (globalLogs.length > 400) globalLogs.splice(0, globalLogs.length - 400);
            // 仅当运行日志明确出现“连接超时”时触发下线提醒（避免其它错误刷屏）
            const msgText = String(logEntry.msg || '');
            if (msgText && msgText.includes('连接超时')) {
                triggerOfflineReminder({
                    accountId,
                    accountName: worker.name,
                    reason: 'log:connection_timeout',
                    logMsg: msgText,
                });
            }
            ...
            // 账号状态异常：WS 连接被拒绝/登录失效（worker 侧仅上报 400），触发下线提醒
            triggerOfflineReminder({
                accountId,
                accountName: worker.name,
                reason: `status:ws_error:${code || 'unknown'}`,
                message,
            });
            if (code === 400) {
                addAccountLog(
    ...
    function callWorkerApi(accountId, method, ...args) {
        ...
        return new Promise((resolve, reject) => {
            ...
            // 超时处理（好友过多时 getFriends 等可能较慢，放宽至 60 秒）
            managerScheduler.setTimeoutTask(`api_timeout_${accountId}_${id}`, 60000, () => {
