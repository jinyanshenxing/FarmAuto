...
function createRuntimeState(options) {
    ...
    function buildConfigSnapshotForAccount(accountId) {
        return {
            ...
            stealBlacklist: store.getStealBlacklist(accountId),
            stealDelaySeconds: store.getStealDelaySeconds(accountId),
            friendOpOrder: (typeof store.getFriendOpOrder === 'function' ? store.getFriendOpOrder(accountId) : ['help', 'steal', 'bad']),
            plantOrderRandom: store.getPlantOrderRandom(accountId),
            fertilizerOrderRandom: (typeof store.getFertilizerOrderRandom === 'function' ? store.getFertilizerOrderRandom(accountId) : false),
    ...
    function log(tag, msg, extra = {}) {
        ...
        if (globalLogs.length > 400) globalLogs.splice(0, globalLogs.length - 400);
    ...
    function addAccountLog(action, msg, accountId = '', accountName = '', extra = {}) {
        ...
        if (accountLogs.length > 150) accountLogs.splice(0, accountLogs.length - 150);
        runtimeEvents.emit('account_log', entry);
    }

    const PANEL_FRIEND_SCHEDULE_MAX = 150;   // 面板展示的好友调度条数上限
    const PANEL_FRIEND_CROPS_MAX = 10;        // 每条调度展示的作物数上限
    ...
    function normalizeStatusForPanel(data, accountId, accountName) {
        ...
        let friendSchedule = src.friendSchedule;
        if (Array.isArray(friendSchedule) && friendSchedule.length > 0) {
            friendSchedule = friendSchedule.slice(0, PANEL_FRIEND_SCHEDULE_MAX).map((f) => {
                const crops = Array.isArray(f.crops) ? f.crops.slice(0, PANEL_FRIEND_CROPS_MAX) : [];
                return { ...f, crops };
            });
        }
        return {
            ...src,
            friendSchedule: friendSchedule || src.friendSchedule,
            accountId,
            accountName,
            operations: ops,
        };
    ...
    /** 移除指定账号在内存中的运行日志与账号操作日志，用于删除账号时清理残留缓存（不涉及卡密绑定） */
    function purgeLogsForAccount(accountId) {
        const id = String(accountId || '').trim();
        if (!id) return;
        for (let i = globalLogs.length - 1; i >= 0; i--) {
            if (String(globalLogs[i] && globalLogs[i].accountId || '') === id) globalLogs.splice(i, 1);
        }
        for (let j = accountLogs.length - 1; j >= 0; j--) {
            if (String(accountLogs[j] && accountLogs[j].accountId || '') === id) accountLogs.splice(j, 1);
        }
    }
