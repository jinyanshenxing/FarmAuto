...
const { sendQqMail, normalizeQqMailAddress } = require('../services/qqmail');
...
function createReloginReminderService(options) {
    ...
    const offlineNotifyAt = new Map(); // key: `${accountId}:${reason}` => lastAt
    ...
    function shouldSendOfflineReminder(accountId, reason, minIntervalMs = 10 * 60 * 1000) {
        const id = String(accountId || '').trim();
        const r = String(reason || '').trim() || 'offline';
        const key = `${id}:${r}`;
        const now = Date.now();
        const last = offlineNotifyAt.get(key) || 0;
        if (now - last < minIntervalMs) return false;
        offlineNotifyAt.set(key, now);
        return true;
    }

    function resolveAccountQq(accountId) {
        const id = String(accountId || '').trim();
        if (!id) return '';
        const data = typeof getAccounts === 'function' ? getAccounts() : { accounts: [] };
        const list = Array.isArray(data.accounts) ? data.accounts : [];
        const found = list.find(a => a && String(a.id || '') === id);
        if (!found) return '';
        return String(found.qq || found.uin || '').trim();
    }
    ...
    async function triggerOfflineReminder(payload = {}) {
        try {
            ...
            const detail = String(payload.detail || payload.logMsg || payload.message || '').trim();
            if (!channel || !token || !title || !content) return;
            if (!shouldSendOfflineReminder(accountId, reason || channel)) return;
            if (channel === 'webhook' && !endpoint) return;
            if (channel === 'qqmail' && !endpoint) return; // endpoint: from email
            ...
            let ret = null;
            if (channel === 'qqmail') {
                const qq = resolveAccountQq(accountId);
                const to = normalizeQqMailAddress(qq);
                if (!to) return;
                const sendRet = await sendQqMail({
                    from: endpoint, // 发件QQ邮箱
                    authCode: token, // SMTP 授权码
                    to,
                    subject: title,
                    text: content,
                });
                ret = { ok: !!(sendRet && sendRet.ok), msg: sendRet && sendRet.messageId ? sendRet.messageId : 'ok' };
            } else {
                ret = await sendPushooMessage({
                    channel,
                    endpoint,
                    token,
                    title,
